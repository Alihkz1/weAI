...
const valueIsValid = computed(() => {
    if (inputValue.value == null || inputValue.value === "") return false
    return !sameNameExists(inputValue.value)
})
...
<template>
    <div class="relative">
        ...
        <input id="myInput" type="text" :disabled="!canEdit" :value="inputValue"
        ...
        <img v-if="!canEdit" class="absolute right-4 top-4 z-20 cursor-pointer" :src="editIcon" @click="handleEdit" />

        <img v-else-if="valueIsValid === true" class="absolute right-4 top-4 z-20" :src="checkIcon" />
